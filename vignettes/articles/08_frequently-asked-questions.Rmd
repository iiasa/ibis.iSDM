...
This thus requires that users transform them a priori (for instance log transform) before adding them to
the estimation.
...
Cross-validation has been deliberately not integrated into package. Users who would like to 
...
<details>
<summary><b> I am predicting over a large area, is there a way to parallelize the computation?</b>
</summary>

Yes and no.
Generally, the computation speed is handled by the respective engine and not every
engine supports for example multi-threaded computations.
However, the most computationally demanding steps in the package is usually the 
spatial prediction and there are some functionalities to 'tile' up the data over which
...
Now most prediction should make use of the specified future plan. This counts for
both initial model predictions and projections.

If you aim to parallelize over a range of species instead, it might be more worthwhile
to rather parallelize the iteration and not the prediction.
...
Sorry to hear and unless the model or input data are grossly misspecified, this should not happen. Consider changing the input covariates. Often scaling the covariates (see `add_predictors()`) can help with model convergence. Similarly if the use of discrete variables (e.g. ``factor` predictors) results in errors, consider setting the parameter "explode_factors" to TRUE in `add_predictors()`. This will create additional covariates for every factor level.
