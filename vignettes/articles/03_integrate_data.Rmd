...
article_packages <- c("INLA", "inlabru", "glmnet", "glmnetUtils", "xgboost", "terra", "sf", "igraph", "assertthat")
...
* to provide a more comprehensive estimate of the distribution of a species distribution by relying on multiple sources of evidence. Particular when there are multiple surveys that complement each other.
...
The [Package comparison page](07_package_comparison.html) and the examples below provide some insight into what is currently possible with regards to data integration in the ibis.iSDM R-package. Additional methods, priors and engines are planned.
...
Sampling bias is a ubiquitous problem in presence-only biodiversity data, where observations are often clustered near roads, cities, or popular natural sites. The `add_control_bias()` function allows you to specify a covariate that accounts for this spatial bias and partial it out of the prediction. The `bias_value` parameter sets the value at which the bias covariate is held constant during prediction (e.g., `0` to represent minimal bias).

```{r Bias control, fig.width = w, fig.height = h}
...
When there are very few observations relative to the number of predictors, fitting a full model can lead to overfitting. The Ensemble of Small Models (ESM) approach ([Breiner et al. 2015](https://doi.org/10.1111/2041-210X.12403)) addresses this by fitting many bivariate models (one per pair of predictors) and then combining them into an ensemble. This can be enabled via `add_control_esm()`.

```{r ESM, fig.width = w, fig.height = h}
...
```{r Specification of priors, fig.width = w, fig.height = h}
...
See the [Engine comparison](06_engine_comparison.html) for an overview on which engines support which level of integration.
