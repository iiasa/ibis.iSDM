...
article_packages <- c("glmnet", "glmnetUtils", "stars", "terra", "lwgeom", "sf", "igraph", "ggplot2", "ncdf4", "geosphere", "assertthat")
...
```{r Load data, warning=FALSE, fig.width = w, fig.height = h}
...
```{r Plotting and summarizing the created projections, fig.width = w, fig.height = h}
...
* `add_constraint_dispersal()` To add a dispersal constraint to the projections which is applied after each time step. Supports various options with `'sdd_fixed'` for fixed dispersal kernels, `'sdd_nexpkernel'` for a negative exponential kernel or `'kissmig'` for applying the kissmig framework.
...
```{r Add constraints and reproject, fig.width = w, fig.height = h}
...
```{r Summarize area of projections, fig.width = w, fig.height = h}
