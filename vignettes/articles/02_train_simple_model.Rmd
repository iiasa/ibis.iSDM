...
Creating a model in the `ibis.iSDM` package is relatively straightforward which we demonstrate here with some of testdata that come with the package. These data show the distribution of a simulated forest-associated species for northern Europe. There are also some test predictors available for modelling. 
...
It is common practice in species distribution modelling that resulting predictions are *thresholded*, e.g. that an abstraction of the continuous prediction is created that separates the background into areas where the environment supporting a species is presumably suitable or non-suitable. Thresholds can be used in ibis.iSDM via the `threshold()` function supplying either a fitted model, a RasterLayer or a Scenario object.
...
The ibis.iSDM package provides a convenience function to obtain validation results for the fitted models. Validation can be done both for continuous and discrete predictions, where the latter requires a computed threshold fits (see above).
...
### Checking a fitted model for common issues

The `check()` function runs a series of automated diagnostics on model objects, including checks for model convergence, coefficient existence, outlier predictions, and data sufficiency. It is good practice to run `check()` on both the distribution object before fitting and the fitted model object afterwards.

```{r Check model diagnostics}
# Check the fitted model for common issues
check(fit)

# It is also possible to check the distribution object before training
check(mod)
```

### Environmental similarity assessment

When projecting a fitted model to new areas or time periods, it is important to assess whether the environmental conditions fall within the range of the training data. The `similarity()` function computes either the Multivariate Environmental Similarity Surface (MESS) or the multivariate combination novelty index (NT2) to identify areas of environmental extrapolation.

```{r Environmental similarity, fig.width = w, fig.height = h}
# Calculate MESS (Multivariate Environmental Similarity Surface)
sim <- similarity(fit, method = "mess", plot = TRUE)

# The result contains similarity values per variable and an overall summary
names(sim)
```

### Identifying locally limiting factors

Understanding *which* environmental variable most limits the predicted suitability at each location can provide ecological insight. The `limiting()` function computes, for each grid cell, which predictor is most responsible for decreasing suitability relative to the mean environmental conditions.

```{r Limiting factors, fig.width = w, fig.height = h}
# Identify the most limiting factor at each grid cell
lim <- limiting(fit, plot = TRUE)
```

### Saving and loading fitted models

Fitted models can be saved to disk and reloaded later using `write_model()` and `load_model()`. This is useful for sharing results or resuming work without refitting. Spatial predictions can also be exported as GeoTIFF or NetCDF via `write_output()`.

```{r Save and load models, eval = FALSE}
...
```{r Train models with spatial constraints, fig.width = w, fig.height = h}
