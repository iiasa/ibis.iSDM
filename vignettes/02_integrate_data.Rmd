...
invisible( install(c("inlabru", "xgboost")) )
```
...
plot( mod1$predictors$get_data()[["distance_range"]] )
...
* "interaction": In the case of two datasets of the same type it also is possible to make use of factor interactions. In this case the prediction is made based on the first reference level (e.g. the first added dataset). 
