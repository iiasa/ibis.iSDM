...
<details>
<summary><b>I have presence-only data (GBIF and co). How can I model a probability of occurrence? </b>
</summary>
Technically, it is impossible to estimate a probability of occurrence just with presence-only data (such as commonly available from databases like GBIF). What people normally do is to add so called pseudo-absence (often in excessive numbers) over the entire background to their data.

In the `ibis.iSDM` package it is possible to add these pseudo-absence points to a presence-only dataset as follows:
...
Also see the `add_pseudoabsence()` and `pseudoabs_settings()` help pages for more settings and information here.
</details>
...
In addition, for Bayesian Engines other bands quantifying the posterior predictive uncertainty might be available and can be plotted as well. The raster can also be saved a spatial GeoTiff to a given filename.
...
mod$save("myoutput.tif", type = "gtif", dt = "FLT4S")
...
There are various forms of integration from a simple approach of adding [`priors`] or [`offsets`] to fully integrated multiple likelihood models (see [Fletcher et al. 2019](https://doi.org/10.1002/ecy.2710)). Thus, users have a range of possibilities to combine different sources of evidence in their modelling.

With regards to how different [`engines`] treat multiple biodiversity datasets. Unfortunately only the [`engine_inla()`], [`engine_inlabru()`] and [`engine_stan()`] support for fully integrated multiple likelihood models. All other [`engines`] combine multiple datasets by running separate models in sequence where the order is determined by the sequence the datasets were added to the model. The output of each model is then provided as input predictor to the next one.
In the future rather than adding outputs as predictors, there might be an option to specify an offset of the prediction instead. This has yet not been implemented.
...
* Make use of spatial thinning approaches. See for instance [Aiello-Lammers et al. 2015](https://onlinelibrary.wiley.com/doi/full/10.1111/ecog.01132) and [Steen et al. 2021](https://besjournals.onlinelibrary.wiley.com/doi/abs/10.1111/2041-210X.13525). Note however that spatial thinning does remove data points, affecting for instance any `poisson` distributed models (PPMs) in the process. The `ibis.iSDM` package currently has no built-in methods for spatial thinning.
* Partial out a biased variable during the prediction. The `bias_variable` can be specified in the `train()` command and has to be part the predictor set of the model. In addition a `bias_value` has to be specified and this can be set for instance to `0`.
