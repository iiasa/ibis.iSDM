...
#' @param value A numeric value for thresholding or NULL
...
methods::setMethod(
  ...
  function(obj, method = 'fixed', value = NULL) {
    assertthat::assert_that(inherits(obj, "DistributionModel"),
                            is.character(method),
...
.stackthreshold <- function(obj, method = 'fixed', value = NULL,
                            layers = c('mean','X0.05quant','X0.5quant','X0.95quant'), poi = NULL) {
  ...
  out <- raster::stack()
  for(i in wl) out <- raster::addLayer(out, threshold(obj[[i]], method = method, value = value, poi = poi) )
...
methods::setMethod(
  ...
  function(obj, method = 'fixed', value = NULL, poi = NULL) {
    ...
    assertthat::assert_that('mean' %in% names(obj))
