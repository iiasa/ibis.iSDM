...
#' @param region.poly A supplied region.poly object. See INLA documentation.
...
#'
#' @param mesh A inla.mesh mesh object.
#'
#' @returns A [`sf`] object.
#'
#' @keywords utils
...
#'
#' @param mesh A inla.mesh object.
#' @param region.poly A \code{"SpatialPolygons"} (legacy sp R-package) object.
#'
...
coords_in_mesh <- function(mesh, coords) {
  assertthat::assert_that(
    inherits(mesh,'inla.mesh'),
    inherits(coords,'sf') || inherits(coords,'Spatial') || inherits(coords, 'matrix') || inherits(coords, 'data.frame')
  )
...
post_prediction <- function(mod, nsamples = 100,
  ...
  .Deprecated('post_prediction','This function is deprecated and will be removed in future versions. Please use the `predict()` method instead.')
  invisible(NULL)
