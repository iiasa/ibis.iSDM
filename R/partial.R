...
#' @param x.vars A [`character`] indicating the variable for which a partial effect is to be calculated
...
methods::setGeneric(
  "partial",
  signature = methods::signature("mod","x.vars"),
  function(mod, x.vars, constant = NULL, variable_length = 100, plot = FALSE, ...) standardGeneric("partial"))

#' @name partial
#' @rdname partial
#' @usage \S4method{partial}{ANY,character}(mod, x.vars)
methods::setMethod(
  "partial",
  methods::signature(mod = "ANY", x.vars = "character"),
  function(mod, x.vars, constant = NULL, variable_length = 100, plot = FALSE, ...) {
    assertthat::assert_that(!missing(x.vars),msg = 'Specify a variable name in the model!')
    ...
    if(inherits(mod,'DistributionModel')){
      partial.DistributionModel(mod, x.vars, constant, variable_length, plot, ...)
    } else {
...
methods::setGeneric(
  "spartial",
  signature = methods::signature("mod","x.vars"),
  function(mod, x.vars, constant = NULL, variable_length = 100, plot = FALSE, ...) standardGeneric("spartial"))

#' @name spartial
#' @rdname spartial
#' @usage \S4method{spartial}{ANY,character}(mod, x.vars)
methods::setMethod(
  "spartial",
  methods::signature(mod = "ANY", x.vars = "character"),
  function(mod, x.vars, constant = NULL, plot = FALSE, ...) {
    assertthat::assert_that(!missing(x.vars),msg = 'Specify a variable name in the model!')
    assertthat::assert_that(inherits(mod, "DistributionModel"),
                            is.character(x.vars),
                            is.null(constant) || is.numeric(constant),
    ...
    # Work around to call partial response directly
    if(inherits(mod,'DistributionModel')){
      spartial.DistributionModel(mod, x.vars, constant, plot, ...)
    } else {
